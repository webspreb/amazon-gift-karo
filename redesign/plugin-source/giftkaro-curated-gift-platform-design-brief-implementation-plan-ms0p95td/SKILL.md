...
### 1.2 Page B: Gift Detail Page (`/gift/:id`)
...
|  - 4:5 aspect ratio         - Title (FerrariSans, 26px/500)      |
|  - Full bleed, no radius    - Subtitle (Body-Font uppercase)     |
|                             - Price Range (theme accent color)   |
|                             - Description paragraph              |
|                             - "Why It's Great" list (styled bullets)|
|                             - Primary CTA: "View on Amazon →"    |
|                             - Affiliate disclosure (text-xs)     |
|                                                                  |
|  Related Gifts Grid (3–4 cards)                                  |
+------------------------------------------------------------------+
```

**Layout Structure (Mobile):**
- Single column: Image → Content → CTA → Related Gifts
- CTA full-width, sticky bottom on scroll (optional)

**Component Inventory — Gift Detail:**
- `Breadcrumb` — "← Back to [Occasion/Vibe/Category]"
- `ProductImage` — lazy-loaded, 4:5 aspect, object-fit: cover
- `GiftMeta` — title, subtitle, price range (styled pill)
- `DescriptionBlock` — editorial paragraph
- `WhyItsGreat` — styled list with theme-accent bullets (● or custom SVG)
- `AmazonCTA` — primary button, full-width mobile, theme accent bg, white text, 2px radius
- `AffiliateDisclosure` — inline below CTA, text-xs, muted
- `RelatedGiftsGrid` — 3–4 GiftCard components

**Interaction Rules:**
- Amazon CTA opens in new tab (`target="_blank" rel="noopener noreferrer"`)
- Affiliate link format: `https://www.amazon.in/dp/[ASIN]?tag=[ASSOCIATE_TAG]`
- Image zoom on hover/tap (lightbox optional)
- Smooth scroll to related gifts
...
- [ ] Breadcrumb + product image (lazy, 4:5)
- [ ] Content panel: title, subtitle, price range, description, Why It's Great
- [ ] Amazon CTA (primary, theme accent) + affiliate disclosure
- [ ] Related gifts grid
- [ ] **Acceptance:** CTA opens Amazon.in with correct affiliate tag in new tab
...
- [ ] **Asset sourcing:** Who provides product photography? (Need 16:9 hero + 4:5 detail per gift)
- [ ] **CMS/Content:** Static JSON files or headless CMS (Contentful, Sanity, Notion)?
- [ ] **Affiliate ID:** Confirm Amazon Associates tracking ID for production
...
Formalized by Open Design from candidate 5415ea0f-7b78-4e84-b27f-249075317196.
